...
if ([% class_name %]) throw "Double declaration of [% class_name %]";
...
my $class_number = 100;
my $class_name_prefix = 'StressTest';

for (my $i = 1; $i <= $class_number; $i++) {
	my @deps = ();
	
	for (my $j = $i + 1; $j <= $class_number; $j++) {
		#averagely 5 dependencies 
		if (rand() < 5/($class_number - $i)) {
