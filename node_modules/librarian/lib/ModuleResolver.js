...
module.exports = function (Joose) {
    ...
    return Joose.Role({
        ...
        requires    : [ 'fileExists', 'getFileAsJSON', 'getCoreModule' ],
        ...
        methods : {
            ...
            resolveAsDir : function (requireStr) {
                ...
                if (this.fileExists(requireStr + '/package.json')) {
                    var packageJSON = this.getFileAsJSON(requireStr + '/package.json')
            ...
            resolveAsModule : function (requireStr, baseDir) {
                ...
                for (var i = 0; i < dirs.length; i++) {
                    ...
                    var resolved    = this.resolveAsDir(dirs[ i ] + '/' + requireStr)
            ...
            resolve : function (requireStr, baseDir) {
                ...
                if (/^\//.test(requireStr))             return this.resolveAsFile(requireStr) || this.resolveAsDir(requireStr)
                if (/^\.\.?\//.test(requireStr))  {
                    var path    = baseDir + '/' + requireStr
                    
                    return this.resolveAsFile(path) || this.resolveAsDir(path)
                }
