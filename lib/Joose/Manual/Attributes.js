...
Joose attributes have many properties, and attributes are probably the single most powerful and flexible part of Joose. You can create a powerful class simply by declaring attributes. In fact, it's possible to have classes that consist solely of attribute declarations.
...
The options passed to has define the properties of the attribute. There are many options, but in the simplest form you just need to set is, which can be either rw (read-write) or ro (read-only). When an attribute is rw, you can change it by passing a value to its accessor. When an attribute is ro, you may only read the current value of the attribute.

In fact, you could even omit is, but that gives you an attribute that has no accessor. This can be useful with other attribute options, such as handles. However, if your attribute generates no accessors, Joose will issue a warning, because that usually means the programmer forgot to say the attribute is read-only or read-write. If you really mean to have no accessors, you can silence this warning by setting is to bare.
Accessor methods
...
If you're thinking that doing this over and over would be insanely tedious, you're right! Fortunately, Joose provides a powerful extension system that lets you override the default naming conventions. See Joose.Manual.JooseX for more details.
Predicate and clearer methods

Joose allows you to explicitly distinguish between a false or undefined attribute value and an attribute which has not been set. If you want to access this information, you must define clearer and predicate methods for an attribute.
...
By default, Joose does not make a predicate or clearer for you. You must explicitly provide names for them, and then Joose will create the methods for you.
Required or not?
...
Attributes can have default values, and Joose provides two ways to specify that default.
...
Joose will throw an error if you pass a bare non-subroutine reference as the default.

If Joose allowed this then the default mapping attribute could easily end up shared across many objects. Instead, wrap it in a subroutine reference:
...
Roles are covered in Joose.Manual.Roles.
Laziness and lazy_build

Joose lets you defer attribute population by making an attribute lazy:
...
Note the doubled underscore in the builder name. Internally, Joose simply prepends the attribute name with "_build_" to come up with the builder name.
...
Options that you explicitly provide are always used in favor of Joose's internal defaults.
...
Joose has built-in support for weak references. If you set the weak_ref option to a true value, then it will call Scalar.Util.weaken whenever the attribute is set:
...
The trigger is called after an attribute's value is set. It is called as a method on the object, and receives the new and values as its arguments. If the attribute had not previously been set at all, then only the new value is passed. This lets you distinguish between the case where the attribute had no value versus when it was undef.
...
Joose also provides a shortcut for specifying that an attribute only accepts objects that do a certain role:
...
See the Joose.Manual.Types documentation for a complete discussion of Joose's type system.
Delegation
...
See Joose.Manual.Delegation for documentation on how to set up delegation methods.
Metaclass and traits

One of Joose's best features is that it can be extended in all sorts of ways through the use of custom metaclasses and metaclass traits.

When declaring an attribute, you can declare a metaclass or a set of traits for the attribute:

  use JooseX.AttributeHelpers;
...
In this case, the metaclass Collection.Hash really refers to JooseX.AttributeHelpers.Collection.Hash.

You can also apply one or more traits to an attribute:

  use JooseX.MetaDescription;

  has 'size' => (
      is          => 'ro',
      traits      => ['JooseX.MetaDescription.Meta.Trait'],
      description => {
...
There are a number of JooseX modules on CPAN which provide useful attribute metaclasses and traits. See Joose.Manual.JooseX for some examples. You can also write your own metaclasses and traits. See the "Meta" and "Extending" recipes in Joose.Cookbook for examples.
...
Joose attributes are a big topic, and this document glosses over a few aspects. We recommend that you read the Joose.Manual.Delegation and Joose.Manual.Types documents to get a more complete understanding of attribute features.
A FEW MORE OPTIONS ^

Joose has lots of attribute options. The ones listed below are superseded by some more modern features, but are covered for the sake of completeness.
...
Joose does absolutely nothing with this information other than store it.
The auto_deref option

If your attribute is an array reference or hash reference, the auto_deref option will make Joose dereference the value when it is returned from the reader method:

  my %map = $object->mapping;

This option only works if your attribute is explicitly typed as an ArrayRef or HashRef.

However, we recommend that you use JooseX.AttributeHelpers for these types of attributes, which gives you much more control over how they are accessed and manipulated.
Initializer

Joose provides an attribute option called initializer. This is similar to builder, except that it is only called during object construction.

This option is inherited from Class.MOP, but we recommend that you use a builder (which is Joose-only) instead.
AUTHOR ^
