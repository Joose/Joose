        ...
        pack: function (object, seen) {
            ...
            Joose.O.eachSafe(attrs, function packAttr (attr, name) {
                if(attr.isPersistent()) {
                    var attrValue = object[name];
                    if (attrValue.meta) {
                       	// the attribute stores another Joose Object
                       	if (attrValue.meta.does(Joose.Storage)) {
                            o[name] = attrValue.pack(seen);
                        } else {
                            throw new ReferenceError('Attempt to pack an attribute of type Joose.Obj that does not do the storage role');
                        }
                    } else {
                        //ok now handle the attributes of objects and arrays
                        if (typeof attrValue == 'object'
                            || typeof attrValue == 'array' ) {
                            if (typeof attrValue == 'object')
                                var newAttr = {};
                            if (attrValue instanceof Array)
                                var newAttr = [];
                            for (i in attrValue) {
                                var subAttr = attrValue[i];
                                if (subAttr.meta) {
                                    if (subAttr.meta.does(Joose.Storage)) {
                                        newAttr[i] = subAttr.pack(seen);
                                    } else {
                                        throw new ReferenceError('Attempt to pickle an attribute containing objects of type Joose.Obj that does not do the storage role');
                                    }
                                } else {
                                    newAttr[i] = subAttr;
                                }
