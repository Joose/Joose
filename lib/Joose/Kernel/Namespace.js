    ...
    classMethods: {
        ...
        prepareNamespace: function (name) {
            ...
            for(var i = 0; i < parts.length; i++) {
                ...
                var containerCur = object.meta.container[part];
                var namespaceCur = object.meta.namespaceElements[part];
                
                if (containerCur === undefined && namespaceCur === undefined) {
                    var newKeeper = new Joose.Kernel.NamespaceKeeper(name).getClassObject();
                    object.meta.addElement(newKeeper.meta.localName, newKeeper);
                } else if (containerCur && !namespaceCur) {
                    throw "Trying to setup module "+name+" failed. There is already something: "+containerCur
                } else if (containerCur && !containerCur.meta && namespaceCur.meta.meta.isa(Joose.Kernel.NamespaceKeeper)) {
                    //container was possible changed during loading, return meta back
                    containerCur.meta = namespaceCur.meta;
                }               
//                else {
//                    //XXX should we try to quietly setup a meta (Joose.Kernel.NamespaceKeeper instance) if its absent?
//                    if(!(containerCur && containerCur.meta && containerCur.meta.meta && containerCur.meta.meta.isa(Joose.Kernel.Namespace))) {
//                        throw "Trying to setup module "+name+" failed. There is already something: "+containerCur
//                    }
//                }
