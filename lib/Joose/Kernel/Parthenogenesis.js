...
parthenogenesisMeta.addGenes({
	...
	checkIsa : function (classObject) {
		for (var i = 0; i < this.parentClasses.length; i++) {
			if (this.parentClasses[i] == classObject || this.parentClasses[i].meta.checkIsa(classObject)) return true;
		}
    ...
    isa : function (classObject) {
        ...
        for(var i = 0; i < this.parentClasses.length; i++) {
            if(this.parentClasses[i].meta.checkIsa(classObject)) return true;
        }
