//inheritableMeta will separate meta and parent classes
var inheritableMeta = new Joose.Kernel.ProtoMeta(function(name, c) {
    //this is the actual separation - see below for overriden initialize method
    this.initialize.apply(this, arguments);
    
    //During instantiation Joose.Kernel.Inheritable needs to introspect itself, so it uses an "initialize" method from previous bootstraping stage in own scope
    //this is a "proto-analog" of parent's constructor calling
    this.meta.initialize.call(this, this.c);
});


//plus some new genes
inheritableMeta.extend({
    ...
    initialize: function(name, c) {
        ...
        this.c = c || this.defaultClassFunctionBody();
...
//further classes have call to adoptNextGeneration in initialize, but inheritableMeta isa Joose.Kernel.ProtoMeta
//and it needs to do it manually
inheritableMeta.adoptNextGeneration();
//the same about _name
inheritableMeta._name = 'Joose.Kernel.Inheritable';

//Joose.Kernel.Inheritable can be used as parent class (supports getMethodNames etc)
Joose.Kernel.Inheritable = inheritableMeta.getClassObject();
