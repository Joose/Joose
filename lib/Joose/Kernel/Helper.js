__global__.addProperty('Class', function(name, props){
     ...
     * purposes. */
    else if(props && props.isa && props.isa != Joose.Kernel.Class) {
        metaClass = props.isa.meta.constructor
    } else {
        /* Default meta class is Joose.Class */
        metaClass   = Joose.Kernel.Class;
    }    
    
    return Joose.Kernel.Namespace.Manager.my.create(name, props, metaClass); 
});


__global__.addProperty('Role', function(name, props){
    return Joose.Kernel.Namespace.Manager.my.create(name, props, Joose.Managed.Role); 
});


__global__.addProperty('Module', function(name, props){
    return Joose.Kernel.Namespace.Manager.my.create(name, props, Joose.Kernel.Namespace.Keeper); 
});
